...
class PromptModel(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        #print('input_batch[attention_mask]:', input_batch['attention_mask'].shape)
        #print('inputs_embeds.shape:', input_batch['inputs_embeds'].shape)
...
class PromptForClassification(nn.Module):
    ...
    def __init__(self,
        ...
        self.classifier = nn.Linear(1024, len(self.verbalizer.label_words))
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        #print('self.verbalize:', self.verbalizer)
        #outputs = outputs.logits
        #--outputs = self.verbalizer.gather_outputs(outputs=outputs)
        #print('outputs:', type(outputs))
        #outputs_at_mask = self.extract_at_mask(outputs, batch)
        #self.soft_embeds = nn.Parameter(soft_embeds, requires_grad=True).cuda()
        label_words_logits = self.classifier(outputs)
        #label_words_logits = torch.matmul(outputs, self.soft_embeds) 
