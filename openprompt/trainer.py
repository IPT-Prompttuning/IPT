...
class BaseRunner(object):
    ...
    def steps_per_epoch(self) -> int:
        ...
        #print('self.config.environment.num_gpus:', self.config.environment.num_gpus)
        #print('self.config.train.gradient_accumulation_steps:', self.config.train.gradient_accumulation_steps)
        ...
        if hasattr(template_config, "optimize") and template_config.optimize is not None: # TODO should add optimize config in each yaml
            if not hasattr(self.inner_model.template, "optimize"):
                ...
                #for name, p in self.inner_model.template.named_parameters():
                        ...
                        #print("111111111")
                        #print(name)
                if config.soft_template.fix_emb == True:
                    optimizer_grouped_parameters = [
                        #{'params': [p for name, p in self.inner_model.template.named_parameters() if 'raw_embedding' not in name]}
                        {'params': [p for name, p in self.inner_model.template.named_parameters() if 'raw_embedding' not in name and 'soft_embeds' not in name]}
                    ]
                else:
                     optimizer_grouped_parameters = [
                        {'params': [p for name, p in self.inner_model.template.named_parameters() if 'raw_embedding' not in name]}
    ...
    def load_checkpoint(self, ckpt: str, load_state = True) -> bool:
        ...
        try:
            #print(self.checkpoint_path(ckpt))
            state_dict = torch.load(self.checkpoint_path(ckpt), pickle_module = dill)
    ...
    def training_epoch(self, epoch):
        ...
        logger.info(f"Global step {self.steps_per_epoch}")
        logger.info(f"Training epoch {epoch}, num_steps {self.global_step}, avg_loss: {total_loss/self.steps_per_epoch:.4f}, total_loss: {total_loss:.4f}")
        return 1
    ...
    def fit(self, ckpt: Optional[str] = None):
        ...
        for self.cur_epoch in range(self.cur_epoch, self.num_epochs):
            ...
            if self.best_score is None or ((score - self.best_score) > 0) == self.config.checkpoint.higher_better:
                ...
                #print(self.model.module.prompt_model.template.soft_embeds)
                #if os.path.exists('./prompt-ptr/prompt.pt'):
                    #os.remove('./prompt-ptr/prompt.pt')  
                #torch.save(self.model.module.prompt_model.template.soft_embeds,'./prompt-ptr/prompt.pt')        
                #if score > 55: 
